...
$stmt = $pdo->prepare("SELECT cart.id as cart_id, ebooks.ebook_id, cart.quantity, ebooks.price 
                        FROM cart 
...
if (count($cart_items) > 0) {
    ...
    $order_stmt = $pdo->prepare("INSERT INTO orders (user_id, total_price, order_status, created_at) VALUES (?, ?, 'Pending', NOW())");
    $order_stmt->execute([$user_id, $total_price]);
    $order_id = $pdo->lastInsertId(); // ดึง order_id ที่เพิ่งสร้าง

    // เพิ่มข้อมูลรายการสั่งซื้อในตาราง order_items
    foreach ($cart_items as $item) {
        $order_item_stmt = $pdo->prepare("INSERT INTO order_items (order_id, ebook_id, quantity, price) VALUES (?, ?, ?, ?)");
