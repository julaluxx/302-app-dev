...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE ebooks SET name = ?, description = ?, image = ?, pdf_file = ?, price = ? WHERE ebook_id = ?");
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="name" name="name"
                    value="<?php echo htmlspecialchars($ebook['name']); ?>" required>
            </div>
            <div class="mb-3">
                <label for="description" class="form-label">Description</label>
                <textarea class="form-control" id="description" name="description"
                    required><?php echo htmlspecialchars($ebook['description']); ?></textarea>
            </div>
            <div class="mb-3">
                <label for="image" class="form-label">Image</label>
                <input type="file" class="form-control" id="image" name="image">
                <?php if ($ebook['image']): ?>
                <img src="../images/<?php echo htmlspecialchars($ebook['image']); ?>" alt="Current Image" width="100"
                    class="mt-2">
                <?php endif; ?>
            </div>
            <div class="mb-3">
                ...
                <a href="../pdfs/<?php echo htmlspecialchars($ebook['pdf_file']); ?>" target="_blank"
                    class="d-block mt-2">View Current PDF</a>
                <?php endif; ?>
            </div>
            <div class="mb-3">
                <label for="price" class="form-label">Price</label>
                <input type="text" class="form-control" id="price" name="price"
                    value="<?php echo htmlspecialchars($ebook['price']); ?>" required>
            </div>
            <div class="mb-3">
                <label for="created_at" class="form-label">Created At</label>
                <input type="text" class="form-control" id="created_at" name="created_at"
                    value="<?php echo htmlspecialchars($ebook['created_at']); ?>" disabled>
            </div>
