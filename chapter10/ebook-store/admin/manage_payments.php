...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'confirm') {
        ...
        $stmt = $pdo->prepare("UPDATE payments SET payment_status = 'Confirmed' WHERE payment_id = ?");
        $stmt->execute([$payment_id]);
        $_SESSION['success_message'] = "Payment Confirmed!";
    } elseif ($action == 'reject') {
        // อัปเดตสถานะ payment_status เป็น Rejected
        $stmt = $pdo->prepare("UPDATE payments SET payment_status = 'Rejected' WHERE payment_id = ?");
        $stmt->execute([$payment_id]);
...
$stmt = $pdo->prepare("SELECT payments.*, orders.user_id, users.username, orders.total_price 
                        FROM payments 
